...
class Pager
{
	...
	*/
	...
	{//constructor sets stage by adding variables to object
		...
		// XXX added bounds check so negative page requests can't be submitted.
		// XXX also added INT(pg) so floats don't get included in our SQL request
		if(isset($_GET['pg']) && is_numeric($_GET['pg']) && $_GET['pg'] >= 1){$this->pageNum = intval($_GET['pg']);}else{$this->pageNum = 1;}
	}
	...
	public function showNav($prefix='<nav>',$suffix='</nav>')
